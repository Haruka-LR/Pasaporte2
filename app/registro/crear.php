...
(function () {
    ...
    const btnSelAll  = document.getElementById('btn-sel-todos');
    const btnDesel   = document.getElementById('btn-desel-todos');

    function getChks() {
        return document.querySelectorAll('.chk-usuario');
    }
    ...
    function actualizarEstadoMaestro() {
        ...
        if (chkTodos && todos.length > 0) {
            chkTodos.checked = seleccionados === todos.length;
            chkTodos.indeterminate = seleccionados > 0 && seleccionados < todos.length;
        }
    }

    if (chkTodos) {
        chkTodos.addEventListener('change', function () {
            getChks().forEach(c => c.checked = this.checked);
        });
    }

    if (btnSelAll) {
        btnSelAll.addEventListener('click', function () {
            getChks().forEach(c => c.checked = true);
            if (chkTodos) chkTodos.checked = true;
            if (chkTodos) chkTodos.indeterminate = false;
        });
    }
    ...
    document.querySelectorAll('.fila-disponible').forEach(function (fila) {
        ...
        fila.addEventListener('click', function (e) {
            ...
            const chk = fila.querySelector('.chk-usuario');
            if (chk) {
    ...
    document.querySelectorAll('.chk-usuario').forEach(function (chk) {
        chk.addEventListener('change', actualizarEstadoMaestro);
    });
